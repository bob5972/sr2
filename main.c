...
static void MainMutateParams(FleetAIType aiType, MBRegistry *mreg)
{
    if (Fleet_IsFlockFleet(aiType) ||
        ...
        MainMutationFParams vf[] = {
            ...
            { "gatherRange",          10.0f, 500.0f,  0.05f, 0.15f, 0.05f},
            { "evadeStrictDistance",  -1.0f, 500.0f,  0.05f, 0.15f, 0.05f},
            { "evadeRange",           -1.0f, 500.0f,  0.05f, 0.15f, 0.05f},
            { "attackRange",          -1.0f, 700.0f,  0.05f, 0.15f, 0.05f},
            { "guardRange",           -1.0f, 500.0f,  0.05f, 0.15f, 0.05f},

            { "creditReserve",       100.0f,1000.0f,  0.05f, 0.15f, 0.01f},
            { "baseSpawnJitter",       1.0f, 100.0f,  0.05f, 0.15f, 0.01f},
            { "fighterFireJitter",     1.0f,  10.0f,  0.05f, 0.15f, 0.01f},

            { "sensorGrid.staleCoreTime",
                                       0.0f, 100.0f,  0.05f, 0.15f, 0.05f},
            { "sensorGrid.staleFighterTime",
                                       0.0f, 100.0f,  0.05f, 0.15f, 0.05f},

            { "flockRadius",          10.0f, 500.0f, 0.05f, 0.15f, 0.05f},
            { "flockCrowding",         0.0f,  20.0f, 0.05f, 0.15f, 0.05f},
            { "alignWeight",          -1.0f,   1.0f, 0.05f, 0.15f, 0.05f},
            { "cohereWeight",         -1.0f,   1.0f, 0.05f, 0.15f, 0.05f},

            { "separateRadius",        5.0f, 500.0f, 0.05f, 0.15f, 0.05f},
            { "separatePeriod",        0.0f,2000.0f, 0.05f, 0.15f, 0.05f},
            { "separateScale",         0.0f, 500.0f, 0.05f, 0.15f, 0.05f},
            { "separateWeight",       -1.0f,   1.0f, 0.05f, 0.15f, 0.05f},

            { "edgeRadius",            1.0f, 500.0f, 0.05f, 0.15f, 0.05f},
            { "edgesWeight",          -0.2f,   1.0f, 0.05f, 0.15f, 0.05f},
            { "centerRadius",          0.0f, 900.0f, 0.05f, 0.15f, 0.05f},
            { "centerWeight",         -0.1f,   0.1f, 0.05f, 0.15f, 0.02f},

            { "coresRadius",           0.0f, 900.0f, 0.05f, 0.15f, 0.05f},
            { "coresWeight",          -1.0f,   1.0f, 0.05f, 0.15f, 0.05f},
            { "coresCrowdRadius",      0.0f, 900.0f, 0.05f, 0.15f, 0.01f},
            { "coresCrowding",        -1.0f,  20.0f, 0.05f, 0.15f, 0.01f},

            { "baseRadius",           10.0f, 500.0f, 0.05f, 0.15f, 0.02f},
            { "baseWeight",           -1.0f,   0.3f, 0.05f, 0.15f, 0.02f},
            { "nearBaseRadius",        1.0f, 500.0f, 0.05f, 0.15f, 0.02f},
            { "baseDefenseRadius",     1.0f, 500.0f, 0.05f, 0.15f, 0.02f},

            { "enemyRadius",           0.0f, 900.0f, 0.05f, 0.15f, 0.05f},
            { "enemyWeight",          -1.0f,   1.0f, 0.05f, 0.15f, 0.05f},
            { "enemyCrowdRadius",      0.0f, 900.0f, 0.05f, 0.15f, 0.02f},
            { "enemyCrowding",        -1.0f,  20.0f, 0.05f, 0.15f, 0.02f},

            { "enemyBaseRadius",       0.0f, 900.0f, 0.05f, 0.15f, 0.02f},
            { "enemyBaseWeight",      -1.0f,   1.0f, 0.05f, 0.15f, 0.02f},

            { "curHeadingWeight",     -1.0f,   2.0f, 0.05f, 0.15f, 0.05f},

            { "attackSeparateRadius",  1.0f, 500.0f, 0.05f, 0.15f, 0.05f},
            { "attackSeparateWeight", -1.0f,   1.0f, 0.05f, 0.15f, 0.05f},

            { "locusRadius",          1.0f, 12345.0f, 0.05f, 0.15f, 0.05f},
            { "locusWeight",         -1.0f,    1.0f,  0.05f, 0.15f, 0.05f},
            { "locusCircularPeriod", -1.0f, 12345.0f, 0.05f, 0.15f, 0.05f},
            { "locusCircularWeight", -1.0f,    1.0f,  0.05f, 0.15f, 0.05f},
            { "locusLinearXPeriod",  -1.0f, 12345.0f, 0.05f, 0.15f, 0.05f},
            { "locusLinearYPeriod",  -1.0f, 12345.0f, 0.05f, 0.15f, 0.05f},
            { "locusLinearWeight",   -1.0f,    1.0f,  0.05f, 0.15f, 0.05f},
            { "locusRandomWeight",   -1.0f,    1.0f,  0.05f, 0.15f, 0.05f},
            { "locusRandomPeriod",   -1.0f, 12345.0f, 0.05f, 0.15f, 0.05f},
        };

        MainMutationFParams bvf[] = {
            // key                     min    max      mag   jump   mutation
            { "holdCount",             1.0f, 200.0f,  0.05f, 0.15f, 0.05f},
            { "holdFleetSpawnRate",   0.01f,   1.0f,  0.05f, 0.15f, 0.05f},
        };
